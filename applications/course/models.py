...
class Course(models.Model):
    ...
    course_description = models.CharField(max_length=100, default="description")
    course_video = models.FileField(upload_to="course/course_video/",                             
                                     null= True,  default=" ")
    course_price_id = models.CharField(max_length=100,  default=" ")
    assisitant_instructor_id = models.ForeignKey(CustomUser, on_delete=models.PROTECT, related_name ="assistant", null= True)
    course_type = EnumField(choices=CourseType , to_choice=lambda x: (x.value, x.name), to_repr=lambda x: x.value)
    course_level = EnumField(choices=CourseLevel , to_choice=lambda x: (x.value, x.name), to_repr=lambda x: x.value)
